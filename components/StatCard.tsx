...
const StatCard: React.FC<StatCardProps> = ({ 
  ...
  return (
    <div className="bg-brand-surface p-3 md:p-6 rounded-2xl flex items-start gap-3 md:gap-5 shadow-lg border border-brand-border">
      <div className={`w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center flex-shrink-0 ${iconBgColor} ${iconColor}`}>
        {icon}
      </div>
      <div className="flex-grow overflow-hidden min-w-0">
        <p className="text-sm text-brand-text-secondary font-medium truncate">{title}</p>
        <div className="flex items-end gap-2 flex-wrap">
            <p className="text-xl md:text-2xl lg:text-3xl font-bold text-brand-text-light break-words leading-tight">{value}</p>
            {change && (
                <div className={`flex items-center text-xs sm:text-sm font-semibold ${changeColor}`}>
                    {changeType === 'increase' ? <TrendingUpIcon /> : <TrendingDownIcon />}
                    <span className="ml-0.5">{change}</span>
                </div>
            )}
        </div>
        {subtitle && <p className="text-xs sm:text-sm text-brand-text-secondary mt-2">{subtitle}</p>}
      </div>
    </div>
  );
};
